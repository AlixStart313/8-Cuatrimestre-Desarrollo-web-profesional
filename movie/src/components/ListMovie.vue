    ...
    <section class="container my-5">
      <div>
        <div class="row mb-3">
          ...
          <div class="col-md-4">
            <select placeholder="Seleccione un genero" class="form-control" v-model="selectedGenre">
            ...
            <b-sidebar id="sidebar-right" title="Agregar Película" right shadow v-model="showSidebar">
              <div class="px-3 py-2">
                ...
                <div class="mb-3">
                  ...
                  <select class="form-control" id="movieGenre" v-model="newMovie.gender">
                    <option v-for="genre in genres" :key="genre" :value="genre">
                      {{ genre }}
                    </option>
                  </select>
...
export default {
  ...
  methods: {
    ...
    async registerMovie() {
      ...
      const apiUrl = 'http://localhost:8080/api/movies/saveMovie';
   // Datos que deseas enviar en la solicitud
      ...
      fetch(apiUrl, {
        ...
        body: JSON.stringify(this.newMovie) // Convertir los datos a formato JSON
      })
        .then(response => {
          if (response.ok) {
